...
class PetAdoptionTablePlugin {
  function __construct() {
    global $wpdb; //GLOBAL variable to of WP database
    $this->charset = $wpdb->get_charset_collate(); //Look for the charset that WP provided to create TABLE below
    $this->tablename = $wpdb->prefix . "pets"; //Look for the prefix that WP provided to make tablename (ex. wp_{tablename})


    //run onActivate function once when we activate the plugin from admin site
    add_action('activate_new-database-table/new-database-table.php', array($this, 'onActivate')); 


    //run onAdminRefresh when we refresh admin dashboard
    // add_action('admin_head', array($this, 'populateFast'));

    //Create and Delete pet by Administrator
    add_action('admin_post_createpet',array($this,'createPetByAdmin'));
    add_action('admin_post_deletepet',array($this,'deletePetByAdmin'));
