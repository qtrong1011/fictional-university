...
class HeroSlider {
  constructor() {
    ...
    allSlideshow.forEach(function (currentSlideshow) {
...
class MyNotes {
  ...
  makeNoteEditable(thisNote) {
    ...
    thisNote.find(".note-title-field, .note-body-field").removeAttr('readonly').addClass("note-active-field"); //remove readonly attribute
    thisNote.find(".update-note").addClass("update-note--visible");
    thisNote.data("state", "editable");
  }
  makeNoteReadOnly(thisNote) {
    thisNote.find(".edit-note").html('<i class="fa fa-pencil" aria-hidden="true"></i>Edit');
    thisNote.find(".note-title-field, .note-body-field").removeClass('note-active-field').attr("readonly", 'readonly'); //remove readonly attribute
  ...
  createNote(e) {
    var ourNewPost = {
      ...
      'status': 'private' //extra security in client-side by setting new note to be PRIVATE
    };
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
      ...
      error: e => {
        if (e.responseText == 'You have reached your note limit') {
      ...
      this.resultsDiv.html(`
                    <div class="row">
                        <div class="one-third">
                            ...
                            ${results.generalInfo.map(result => `<li><a href="${result.url}">${result.title}</a> ${result.post_type == 'post' ? `by ${result.author_name}` : result.post_type == 'page' ? `by ${result.author_name}` : ''}</li>`).join('')}
                            ${results.generalInfo.length ? '</ul>' : ''}
                        </div>
                        <div class="one-third">
                            <h2 class="search-overlay__section-title">Programs</h2>
                            ${results.programs.length ? '<ul class="link-list min-list">' : `<p>No programs match that search.<a href="${universityData.root_url}/programs">View all programs</a></p>`}
                            ${results.programs.map(result => `<li><a href="${result.url}">${result.title}</a></li>`).join('')}
